...
APIKEY='0a1dffe8b6fed64d97e4a51bdad9124c'
...
function getData {
    ...
    echo " " >> "/tmp/.weather.log"
    echo `date`" ################################" >> "/tmp/.weather.log"
    RESPONSE=`curl -s $URL`
    CODE="$?"
    if [ "$1" = "-d" ]; then
        echo $RESPONSE
    ...
    echo "Response: $RESPONSE" >> "/tmp/.weather.log"
    RESPONSECODE=0
    if [ $CODE -eq 0 ]; then
    ...
    if [ $CODE -ne 0 ] || [ ${RESPONSECODE:=429} -ne 200 ]; then
        if [ $CODE -ne 0 ]; then
            ...
            echo "curl Error $CODE" >> "/tmp/.weather.log"
        else
            ERR_MSG="Conn. Err. $RESPONSECODE"
            echo "API Error $RESPONSECODE" >> "/tmp/.weather.log"
        fi
        ERROR=1
    else
    echo "$RESPONSE" > "/tmp/.weather-last"
    echo `date +%s` >> "/tmp/.weather-last"
    fi
}
function setIcons {
    if [ $WID -le 232 ]; then
